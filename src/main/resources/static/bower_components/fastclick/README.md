...
Same goes for Chrome on Android (all versions) with `admin-scalable=no` in the viewport meta tag. But be aware that `admin-scalable=no` also disables pinch zooming, which may be an accessibility concern.
